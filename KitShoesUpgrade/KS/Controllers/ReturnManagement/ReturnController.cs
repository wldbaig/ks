...
namespace KS.Controllers.Return
{
    public class ReturnController : BaseController
    {
        ...
        public ActionResult DetailReadJP([DataSourceRequest] DataSourceRequest request, int ReturnID)
        {
            var x = db.JPReturnItemDetails.Where(c => c.JPReturnItemID == ReturnID && c.ArticlePairs != 0).Select(c => new ReturnDetViewM()
            {
                ReturnID = c.JPReturnItemID,
                Article = db.JPArticles.FirstOrDefault(o => o.ID == c.ArticleID).ArticleName,
                Color = db.WHArticleDetails.FirstOrDefault(o => o.ID == c.ArticleDetailID).Color.ColorName,
                ReturnDetailID = c.ID,
                Quantity = c.ArticlePairs
        ...
        [RoleSecurity(Permissions.Return, PermissionType.ADD)]
        public ActionResult AddJP()
        {
            return View();
        }

        [HttpPost]
        public ActionResult AddJP(FormCollection collection)
        {
            try
            {
                using (var trans = new System.Transactions.TransactionScope())
                {
                    var returnitems = new JPReturnItem();

                    returnitems.TotalPrice = 0;
                    returnitems.CreatedBy = User.ID;
                    returnitems.AddedOn = DateTime.UtcNow;

                    db.JPReturnItems.Add(returnitems);
                    db.SaveChanges();
                    ...
                    foreach (var item in keys)
                    {
                        if (item.Contains("item-color-ID*"))
                        {
                            ...
                            var retDet = new JPReturnItemDetail();
                            retDet.JPReturnItemID = returnitems.ReturnItemID;
                            retDet.ArticleID = artDet.JPArticleId;
                            retDet.ArticleDetailID = artDet.ID;
                            retDet.ArticlePairs = Convert.ToInt32(collection[item]);
                            retDet.Price = 0;
                            db.JPReturnItemDetails.Add(retDet);
                            db.SaveChanges();
                    ...
                    return RedirectToAction("ReturnJPReciept", new { id = returnitems.ReturnItemID });
                }
            }
            catch (Exception ex)
            {
        ...
        [HttpPost]
        public ActionResult GetArticleDetailforJPReturn(string ArticleID)
        {
            if (ArticleID == "")
            {
                return Json("", JsonRequestBehavior.AllowGet);
        ...
        public ActionResult ReturnJPReciept(int id)
        {
            var ret = db.JPReturnItems.Find(id);
            if (ret == null)
            {
                return HttpNotFound();
            }
            ...
            foreach (var ITEM in ret.JPReturnItemDetails)
            {
                if (ITEM.ArticlePairs != 0)
                {
                    if (Pmodel.PReciept.Any(f => f.Article == ITEM.ArticleID))
        ...
        {
            db.Configuration.ProxyCreationEnabled = false;
            var result = db.JPArticleDetails.Select(c => c.JPArticle).Distinct().ToList();
